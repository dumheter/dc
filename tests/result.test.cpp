...
DTEST(match_rvalue_ok)
{
	...
	dutil::Result<TrackedInt, std::string> okResult = dutil::Ok(std::move(parent));
	float value = std::move(okResult).match(
		[](TrackedInt value){ return 1.f; },
...
DTEST(match_rvalue_err)
{
	...
	float value = std::move(errResult).match(
		[](float ok) { return 1.f; },
...
DTEST(match_const_lvalue_ok)
{
	...
	const dutil::Result<TrackedInt, std::string> okResult = dutil::Ok(std::move(parent));
	float value = okResult.match(
		[](const TrackedInt& value){ return 1.f; },
...
DTEST(match_const_lvalue_err)
{
	...
	const dutil::Result<float, TrackedInt> errResult = dutil::Err(std::move(parent));
	float value = errResult.match(
		[](const float& ok){ return 1.f; },
